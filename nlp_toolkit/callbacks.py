...
class Top_N_Acc(Callback):
    ...
    def __init__(self, seq, top_n=5, attention=False, preprocessor=None):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(len(self.seq)):
            ...
            if self.attention:
                y_pred = y_pred[:, :self.p.label_size]
            y_true = self.p.inverse_transform(y_true)
            y_pred = self.p.inverse_transform(y_pred, 5)
...
class F1score(Callback):
    ...
    def __init__(self, seq, attention=False, preprocessor=None):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(len(self.seq)):
            ...
            label_pred.extend(y_pred)
        assert len(label_pred) == len(label_true)
        f1 = self._calc_f1(y_true, y_pred)
        for i in range(f1.shape[0]):
            label = self.p._label_vocab._id2token[i]
...
class F1score_seq(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        logs['f1'] = np.float64(f1)
        logs['seq_acc'] = np.float64(acc)


def get_callbacks(log_dir=None, valid=None, metric='f1',
                  preprocessor=None, early_stopping=True,
    ...
    if valid is not None:
        if metric == 'top_n_acc':
            ...
            callbacks.append(Top_N_Acc(valid, top_n, attention, preprocessor))
        elif metric == 'f1':
            print('mointor training process using f1 score')
            callbacks.append(F1score(valid, attention, preprocessor))
        elif metric == 'f1_seq':
            print('mointor training process using f1 score and label acc')
            callbacks.append(F1score_seq(valid, preprocessor))
